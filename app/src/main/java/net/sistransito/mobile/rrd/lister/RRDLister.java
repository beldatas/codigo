...
import net.sistransito.mobile.impressaobluetooth.PrintBitmap.RRDPrintBitmap;
import net.sistransito.mobile.rrd.RRDData;
import net.sistrnsitomobile.R;

import java.util.Set;

public class RRDLister extends AppCompatActivity implements OnClickListener,
		BluetoothPrinterListerner {
	private RRDListerExpandableAdapter expandableAdapter;
	private ExpandableListView expandableListView;
	private ImageButton im_btn_back;
	private TextView tvMessage;
	private RelativeLayout rrd_lister_layout;
	...
	private boolean checkCursor() {
		cursor = (DatabaseCreator.getDatabaseAdapterRRD(this)).getRRDCursor();

		return cursor.getCount() > 0;
	}

	private void addNoResultView() {
		rrd_lister_layout = (RelativeLayout) findViewById(R.id.rrd_lister_layout);
		cursor.close();
		...
		if (tvMessage.getParent() == null) {
			rrd_lister_layout.addView(tvMessage);
		}
	}

	private void addResultView() {
		expandableAdapter = new RRDListerExpandableAdapter(cursor,
				RRDLister.this);
		expandableListView = (ExpandableListView) findViewById(R.id.expandableListViewRrd);
	...
	private void setPrintData(final Object mainData, final Object autodeData) {
		if (checkBth()) {
			...
			new Thread(new Runnable() {
				@Override
				public void run() {
					RRDData rrdData = (RRDData) mainData;
					DadosDoAuto autoData = (DadosDoAuto) autodeData;
					RRDPrintBitmap rrdPrintBitmap= new RRDPrintBitmap(RRDLister.this,rrdData,autoData);
					ESCP.ImageToEsc(rrdPrintBitmap.getBitmap(), mBth.Ostream, 8, 8);
					closeBth();
				}
			}).start();
		}
