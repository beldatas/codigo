 ...
 */
package net.sistransito.mobile.impressaobluetooth.bluetooth;
...
public class SmoothBluetooth {
	...
	public interface Listener {
		...
		void onDevicesFound(List<Device> deviceList,
	...
	public SmoothBluetooth(Context context, ConnectionTo connectionTo,
	...
	public boolean isBluetoothAvailable() {
		try {
			if (mBluetoothAdapter == null
					|| mBluetoothAdapter.getAddress().equals(null))
				return false;
		...
		public void onReceive(Context context, Intent intent) {
			...
			if (BluetoothDevice.ACTION_FOUND.equals(action)) {
				...
				if (!deviceExist(device)) {
					mDevices.add(new Device(device.getName(), device
							.getAddress(), false));
				}
	...
	private void connectAction(List<Device> devices, final boolean android,
	...
	public void send(String data, boolean CRLF) {
		if (isServiceAvailable()
			...
			byte printdata[] = null;
			try {
				printdata=data.getBytes("UTF-8");
				
		//ISO-8859-1		
				/// if utf8 not print special charater then change utf 8 to iso ok
			} catch (UnsupportedEncodingException e) {
				// TODO Auto-generated catch block
				e.printStackTrace();
			}
			mBluetoothService.write(printdata);
		}
	}
